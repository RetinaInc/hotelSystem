...
public class EditBookingGUI extends JPanel implements ActionListener{
	...
	private JButton btnCancel, btnSaveChanges, btnAdd, addRow, back, removeRow, saveBtn;
	private JTextField newArrivalDay, newArrivalMonth ,newArrivalYear, txtTotalCost;
	private DefaultTableModel model , modelLeft;
	
	public EditBookingGUI(String usersID, int bookingid, int numGuests, int numRooms, int numNights, double price, Date arrivalD, Date departureD){
		...
		calendar.addPropertyChangeListener(new PropertyChangeListener(){
			...
			public void propertyChange(PropertyChangeEvent p) {
				if(calendar.getDate().after(currentDate.getTime()) && calendar.getDate().before(maxDate.getTime())){
					newArrivalDay.setText(Integer.toString(calendar.getDayChooser().getDay()));
					newArrivalMonth.setText(dateConverter(calendar.getMonthChooser().getMonth()));
					newArrivalYear.setText(Integer.toString(calendar.getYearChooser().getYear()));
				}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource().equals(btnAdd)){
			addRooms = new JPanel();
			addRooms.setBorder(new TitledBorder(null, "Change booking details - Booking ID: " + this.bookingid, TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		if(e.getSource().equals(back)){
			if(JOptionPane.showConfirmDialog(null, "Are you sure you want to cancel? Any changes made will not be saved","Cancel editing a booking"
		...
		if(e.getSource().equals(addRow)){
			if(tableLeft.getSelectedRow() == -1){  //if no row selected
				JOptionPane.showMessageDialog(null, "No room selected to add", "Edit Booking error", JOptionPane.OK_OPTION);
			}
			else{
				Object[] newRow = {modelLeft.getValueAt(tableLeft.getSelectedRow(), 0),modelLeft.getValueAt(tableLeft.getSelectedRow(), 1), modelLeft.getValueAt(tableLeft.getSelectedRow(), 2)};
		...
		if(e.getSource().equals(removeRow)){
			if(tableRight.getSelectedRow() == -1){  //if no row selected
				JOptionPane.showMessageDialog(null, "No room selected to remove", "Edit Booking error", JOptionPane.OK_OPTION);
			}
			else{
				Object[] newRow = {model.getValueAt(tableRight.getSelectedRow(), 0),model.getValueAt(tableRight.getSelectedRow(), 1), model.getValueAt(tableRight.getSelectedRow(), 2)};
		...
		if(e.getSource().equals(saveBtn)){
			if(JOptionPane.showConfirmDialog(null, "Your room choice will be overridden for your booking. Do you wish to continue?","Save Changes"
					,JOptionPane.YES_NO_OPTION)== JOptionPane.YES_OPTION){
	...
	private void getAvailableRooms(String leftORright) {
		...
		availableList = new ArrayList<Object[]>(q.editBookings(arrival, numNights, usersID));
		array2dLeft = availableList.toArray(new Object[availableList.size()][]);
		}
		else{
			availableListRight = new ArrayList<Object[]>(q.getUserBookingInfo(bookingid));
			array2dRight = availableListRight.toArray(new Object[availableListRight.size()][]);
		}
	}
