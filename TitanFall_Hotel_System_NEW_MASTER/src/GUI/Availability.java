...
public class Availability extends JFrame implements ActionListener {
	...
	public Availability(String userID, ArrayList<User> users, Calendar dc,
			int numnights, int numRooms, int numGuests) {
		calDate = dc;
		numNights = numnights;
		numberOfRooms = numRooms;
		numberOfGuests = numGuests;
		...
		MouseListener mouseListener = new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 1) {
					...
					int[] testing = availableList.getSelectedIndices();
					...
					for (int i = 0; i < testing.length; i++) {
						...
						if (roomChoice.contains(roomNumberList[testing[i]])) {
							roomChoice.remove(i);
						} else {
							roomChoice.add(roomNumberList[testing[i]]);
						}
					}
